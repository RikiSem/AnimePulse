<template>
    ...
    <page-template :pages="pages" :user="$attrs.auth.user">
        <template v-slot:content>
            <anime-list-filter @filterData="filterData => this.applyFilter(filterData)" :user-id="this.$props.userId" :statuses-for-user="statusForUser" :anime-statuses="animeStatuses" :filter-route="'library.manga.all'" class="mx-5 my-5 default-border-color-sec p-6"></anime-list-filter>
...
export default {
    ...
    methods:{
        ...
        async getAnime() {
            let response = await (axios.post(route('library.manga.all'), {
                'offset': this.offset,
                'params': this.params,
            }));
